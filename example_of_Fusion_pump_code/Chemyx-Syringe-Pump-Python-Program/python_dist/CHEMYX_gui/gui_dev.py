...
class ChemyxPumpGUI(QDialog):
    ...
    def __init__(self):
        ...
        self.CONNECTION = connect.Connection(port=str(self.serportCBox.currentText()),
                                             baudrate=self.baudCBox.currentText())
    ...
    def initUI(self):
        ...
        connbox.addRow(
            QLabel('Baud Rate (WARNING: Must match Baud Rate specified in pump System Settings)', wordWrap=True),
            self.baudCBox)
    ...
    def sendFromGUI(self):
        ...
        widgets = [self.unitsCBox, self.diameterLineEdit, self.volumeLineEdit, self.flowRateLineEdit,
                   self.delayLineEdit]
        funcref = [self.CONNECTION.setUnits, self.CONNECTION.setDiameter, self.CONNECTION.setVolume,
                   self.CONNECTION.setRate, self.CONNECTION.setDelay]
        values = []
